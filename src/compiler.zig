...
pub fn Compiler(comptime mode: ExecutionMode) type {
    return struct {
        allocator: Allocator,
        memory: *Memory(mode),
        ...
        fn compileConstantExpr(
            ...
            switch (constant.toKey(self.intern_pool)) {
                ...
                .value_string => |string| {
                    const object = try Object(mode).String.create(
                        self.memory,
                        string,
                    );
                    try self.writeConstant(.{ .object = &object.object }, loc);
                },
        ...
        fn compileCondExpr(
            ...
            try self.patchJumps(
                self.scratch.else_jumps.items[else_scratch_top..],
            );
        ...
        fn compileCondExprAsLogicalOp(
...

            {
                ...
                if (is_or) {
                    ...
                    self.scratch.code.items[self.scratch.last_jump] =
                        @intFromEnum(
                            invertComparisonJumpOp(@enumFromInt(byte)),
                        );
        ...
        fn compileForExpr(
            ...
            defer {
                ...
                self.scratch.break_jumps.shrinkRetainingCapacity(
                    break_scratch_top,
                );
        ...
        fn writeConstant(
            ...
            const index = self.module.writeConstant(
                self.allocator,
                value,
            ) catch |err|
                switch (err) {
