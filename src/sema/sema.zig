...
pub const Sema = struct {
    ...
    fn analyzeStmt(
        ...
        return switch (key) {
            ...
            => |@"fn"| try self.analyzeFnStmtAux(
                @"fn".identifier.toLoc(self.ast).toSlice(self.source),
                @"fn".params,
                @"fn".return_type,
                @"fn".body,
                loc,
    ...
    fn analyzeLetStmt(
        ...
        const parsed_expr = let.expr orelse {
            const index = try self.declareVariable(
                key == .let_mut,
                false,
                let.identifier.toLoc(self.ast).toSlice(self.source),
                analyzed_type_opt,
            );
            ...
            if (args_opt) |args| {
                for (args, arg_types.items) |arg, arg_type| {
                    _ = try self.declareVariable(
                        false,
                        true,
                        arg.identifier.toLoc(self.ast).toSlice(self.source),
                        arg_type,
                    );
                }
            }
    ...
    fn analyzeIdentifierType(
        ...
        const name = loc.toSlice(self.source);
    ...
    fn analyzeExpr(
        ...
        const sema_expr = switch (key) {
            ...
            .literal_bool,
            .literal_string,
            => try self.analyzeLiteralExpr(key, loc, evals),
            ...
            .neg_bool,
            .neg_num,
            => |rhs| try self.analyzeUnaryExpr(rhs, loc, evals),
            ...
            .identifier,
            => try self.analyzeVariableExpr(loc, evals),
            ...
            .call,
            => |call| try self.analyzeCallExpr(call, loc, evals),
    ...
    fn analyzeLiteralExpr(
        ...
        const slice = loc.toSlice(self.source);
        const literal_variant: SemaExpr.Kind.Literal = switch (literal) {
            .literal_unit => .unit,
            .literal_int => .{ .int = std.fmt.parseInt(i64, slice, 10) catch unreachable },
            .literal_float => .{ .float = std.fmt.parseFloat(f64, slice) catch unreachable },
            .literal_bool => .{ .bool = slice.len == 4 },
            .literal_string => .{ .string = try self.allocator.dupe(u8, slice[1 .. slice.len - 1]) },
            else => unreachable,
        };
    ...
    fn analyzeUnaryExpr(
        ...
        const sema_type = switch (rhs_key) {
            .neg_bool,
    ...
    fn analyzeAssignmentExpr(
        ...
        const name = binary.lhs.toLoc(self.ast).toSlice(self.source);
        ...
        const right = try self.analyzeExpr(binary.lhs, true);
    ...
    fn analyzeCallExpr(
        ...
        if (arg_types.items.len != call.args.len) {
            return self.semaFailure(
                position,
                .{
                    .arity_mismatch = .{
        ...
        for (arg_types.items, call.args, 1..) |arg_type, arg, index| {
