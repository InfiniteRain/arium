...
const TestTokenizer = @import("TestTokenizer.zig");
const TestToken = TestTokenizer.Token;
...
pub const Setup = struct {
    ...
    pub fn deinit(self: *Setup, allocator: Allocator) void {
...
fn parseTestDirective(self: *TestParser) Error!Directive.Data {
    return .{ .@"test" = try self.parseEnum(Setup.Kind) };
}
...
fn parseExpectDirective(self: *TestParser, tag: Directive.Tag) Error!Directive.Data {
    ...
    const loc: Setup.Expect.Loc = switch (loc_directive.tag) {
...
fn parseExpectingSuccess(source: [:0]const u8) TestParser.Error!struct { Setup, InternPool } {
