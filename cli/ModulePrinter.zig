...
const ModulePrinter = @This();

module: *const Module,
output: *const Output,

pub fn print(module: *const Module, output: *const Output) void {
    var index: usize = 0;

    while (index < module.code.items.len) {
        const locals_count = mem.bytesToValue(u32, module.code.items[index..][0..4]);
        const size = mem.bytesToValue(u32, module.code.items[index + 4 ..][0..4]);
        defer index += size + 8;

        output.printf("=== fn: {}, locals: {}, size: {} ===\n", .{ index, locals_count, size });

        const offset_start = index + 8;
        var ip = offset_start;

        while (ip < offset_start + size) {
            ip += printInstruction(module, output, offset_start, ip);
        }
...
pub fn printInstruction(
    ...
    return switch (op_code) {
        ...
        .if_false,
        .jump,
        => module_printer.printJumpInstructionName(offset_start, offset, .positive),

        .negative_jump,
        => module_printer.printJumpInstructionName(offset_start, offset, .negative),
...
fn printConstantInstructionName(self: *const ModulePrinter, T: type, offset: usize) usize {
...
fn printInstructionName(self: *const ModulePrinter, offset: usize) usize {
    const op_code: OpCode = @enumFromInt(self.module.code.items[offset]);
...
fn printJumpInstructionName(
    ...
    const op_code: OpCode = @enumFromInt(self.module.code.items[offset]);
    const jump_offset = mem.bytesToValue(u16, self.module.code.items[offset + 1 ..][0..2]);

    self.printOpCode(op_code);

    if (direction == .positive) {
        self.output.printf(" to {}\n", .{(offset - offset_start) + jump_offset + 3});
    } else {
...
fn printInstructionWithArgName(self: *const ModulePrinter, T: type, offset: usize) usize {
    const op_code: OpCode = @enumFromInt(self.module.code.items[offset]);
